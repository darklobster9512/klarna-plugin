...
import { slugifyVolksbank, volksbanken } from "./bank.volksbanken.$slug";

export const Route = createFileRoute("/bank/volksbanken/")({
  component: VolksbankenListPage,
});

function VolksbankenListPage() {
  const [query, setQuery] = useState("");
  const logo = bankLogoUrls["volksbanken"];

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return volksbanken;
    return volksbanken.filter((n) => n.toLowerCase().includes(q));
  }, [query]);
          ...
          <div className="flex flex-1 flex-col overflow-y-auto px-8 pb-6 sm:px-10">
            ...
            <ul className="mt-6 divide-y divide-neutral-200">
              {filtered.map((name) => (
                <li key={name}>
                  <Link
                    to="/bank/volksbanken/$slug"
                    params={{ slug: slugifyVolksbank(name) }}
            ...
            {filtered.length === 0 && (
              <p className="mt-6 text-[14px] text-[#373544]">Keine Volksbank gefunden.</p>
            )}
          </div>
        </div>
      </div>
