...
import { slugifySparkasse } from "./bank.sparkassen.$slug";

export const Route = createFileRoute("/bank/sparkassen/")({
  component: SparkassenPage,
});
...
function SparkassenPage() {
  const [query, setQuery] = useState("");
  const logo = bankLogoUrls["sparkassen"];

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return sparkassen;
    return sparkassen.filter((n) => n.toLowerCase().includes(q));
  }, [query]);
          ...
          <div className="flex flex-1 flex-col overflow-y-auto px-8 pb-6 sm:px-10">
            ...
            <ul className="mt-6 divide-y divide-neutral-200">
              {filtered.map((name) => (
                <li key={name}>
                  <Link
                    to="/bank/sparkassen/$slug"
                    params={{ slug: slugifySparkasse(name) }}
            ...
            {filtered.length === 0 && (
              <p className="mt-6 text-[14px] text-[#373544]">Keine Sparkasse gefunden.</p>
            )}
          </div>
        </div>
      </div>
